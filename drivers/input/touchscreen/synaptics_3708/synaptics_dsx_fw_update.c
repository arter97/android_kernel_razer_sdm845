...
static unsigned int fwu_show_config_id(void)
{
    ...
    pr_info("### get_device_config_id=%4x",device_config_id);
...
#endif
    config_version = fwu_show_config_id();
    if(config_version == 0xb7000000)
        pr_err("It does not need update FW");
    else
