...
{
    pr_info("F@TOUCH : [%s] Touch_TestResult =%d\n", __func__,Touch_TestResult);
    return Touch_TestResult;
}

static void fullRawCap_retry(void)
{
    int retval = 0;
    unsigned char timeout = GET_REPORT_TIMEOUT_S * 10;
    unsigned char timeout_count;
    ...
    pr_err("F@TOUCH %s numberOfColums TX(%d) numberOfRows RX(%d)\n", __func__, numberOfColums, numberOfRows);

    if((numberOfColums > CFG_F54_TXCOUNT) ||
            (numberOfRows > CFG_F54_RXCOUNT))
    {
        pr_err("F@TOUCH %s read from device T(%d)/R(%d) greater than settings T(%d)/R(%d)\n",
               __func__,
               numberOfColums,
               numberOfRows,
               CFG_F54_TXCOUNT,
               CFG_F54_RXCOUNT);
        Touch_TestResult = TEST_ERROR_1;
        goto exit;
    }
    ...
    {
        pr_err("F@TOUCH : %s alloc ResultArray memory failed\n", __func__);
        Touch_TestResult = TEST_ERROR_2;
        goto exit;
    }
    ...
    else
    {
        ...
        Touch_TestResult = TEST_ERROR_4;
        goto exit;
    }

    TouchRawdata = filp_open("/data/misc/touch/TPRawdataFile.txt", O_CREAT | O_RDWR, 0660 );
    if(IS_ERR(TouchRawdata))
    {
        pr_err("F@TOUCH : %s open /data/misc/touch/TPRawdataFile.txt failed = %ld\n", __func__, PTR_ERR(TouchRawdata));
        Touch_TestResult = TEST_ERROR_5;
        goto exit;
    }
#endif
    ...
    if (retval < 0)
    {
        Touch_TestResult = TEST_ERROR_10;
        goto exit;
    }

    report_size = f54->report_size;
    pr_err("F@TOUCH %s report_size(%d)\n", __func__, report_size);

    if (!report_size)
    {
        Touch_TestResult = TEST_ERROR_11;
        goto exit;
    }
    buffer = kmalloc(report_size, GFP_KERNEL);
    if (!buffer)
    {
        Touch_TestResult = TEST_ERROR_12;
        goto exit;
    }
    ...
    {
        pr_err("F@TOUCH %s f54->report_data not found\n", __func__);
        Touch_TestResult = TEST_ERROR_13;
        goto exit;
    }
...
exit:
    ...
    if(!IS_ERR(TouchRawdata))
    {
        pr_info("F@TOUCH : %s filp_close(TouchRawdata);\n", __func__);
...
exit:
    ...
    if(Touch_TestResult == TEST_PASS)
        return;
    pr_err("### Touch_TestResult = %d",Touch_TestResult);
    fullRawCap_retry();
